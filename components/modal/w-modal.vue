  ...
  <transition name="fade">
    <div :class="c.backdrop" v-if="showModal" @click.self="emitDismiss" ref="backdropEl" data-test="backdrop">
      <transition name="slide">
        <div v-if="showContent" :class="c.modal" tabindex="-1" aria-modal="true" aria-labelledby="w-modal-title" role="dialog" ref="modalEl">
          <div :class="[c.title, headerClasses]">
            <transition-group name="w-modal-title">
              ...
              <div :class="titleCenterClasses" key="title" v-bind="titleAttrs">
                <p id="w-modal-title" :class="c.titleText" v-if="title">{{ title }}</p>
                <slot name="title" />
              </div>
