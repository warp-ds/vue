...
describe('button', () => {
  ...
  test('primary', () => {
    ...
    assert.equal(wrapper.text(), 'Hello Warp')
    assert.include(wrapper.classes().join(' '), 'py-12 px-16 border-0 font-bold rounded-8 leading-24 max-w-max focusable justify-center transition-colors ease-in-out i-text-$color-button-primary-text hover:i-text-$color-button-primary-text i-bg-$color-button-primary-background hover:i-bg-$color-button-primary-background-hover! active:i-bg-$color-button-primary-background-active')
  })
  test('href', () => {
